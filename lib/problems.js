...
function lucasNumber(n) {
    ...
    let ln = lucasNumber(n-1) + lucasNumber(n-2)
    return ln
}
...
function reverseString(str) {
    ...
    let newBeg = str.slice(-1);
    let newStr = newBeg + reverseString(str.slice(0,-1));
    return newStr;
}
...
function pow(base, exponent) {
    if (exponent === 0){
        ...
        let power = 1.0 / (base * pow(base, -(exponent + 1)))
        return power
    } else {
        let power = base * pow(base, exponent - 1)
        return power
    }
}
